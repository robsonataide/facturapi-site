...
function Logo({ dark = false }: { dark?: boolean }) {
  const bg = dark ? "rgba(255,255,255,0.12)" : NAVY;
  const wordmark = dark ? "white" : NAVY;
  return (
    ...
    <footer style={{ background: NAVY, padding: "40px 24px" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          <Logo dark />
          <span style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>© 2026 Todos os direitos reservados.</span>
        </div>
